...
bool sendData(unsigned char whereToSend, struct ringBufS *_ringBuff)
{
   
    //calculate the crc
    unsigned char CS = CRC8(_ringBuff->buf, _ringBuff->count);
    
    serial_write(0x06); //start address
    serial_write(0x85); //start address
    serial_write(whereToSend);
    serial_write(moduleAddress);
    serial_write(_ringBuff->count); //length of packet not including the crc


    //send the rest of the packet
    int i;
    for (i = 0; i < _ringBuff->count; i++)
    {
        serial_write(*(_ringBuff->buf + i));
    }
    ...
    FastTransfer_buffer_flush(_ringBuff, 1);
    return true;
}
bool sendData_debug(unsigned char whereToSend, struct ringBufS *_ringBuff)
{

    //calculate the crc
    unsigned char CS = CRC8(_ringBuff->buf, _ringBuff->count);

    Send_put_debug(0x06); //start address
    Send_put_debug(0x85); //start address
    Send_put_debug(whereToSend);
    Send_put_debug(moduleAddress);
    Send_put_debug(_ringBuff->count); //length of packet not including the crc
    ...
    for (i = 0; i < _ringBuff->count; i++)
    {
        Send_put_debug(*(_ringBuff->buf + i));
    }
...
//    {
//        DRV_USART0_WriteByte(serial_read());
//    }
    ...
    if (rx_len != 0)
    {
        ...
        if (rx_array_inx == 0)
        {
            ...
            if (255 == serial_peek())
            {
                ...
                wipeBuf(rx_buffer,255);
                return receiveData();
            }
        }

        
        while ((serial_available()) > 0 && ( rx_array_inx <= rx_len))//(serial_available() > 0) &&( rx_array_inx <= rx_len))
        {  
        ...
        if (rx_len == (rx_array_inx - 1))
        {
            ...
            {//CS good
                ...
                useFastTransData(receiveArrayAddress);
...
void ToSend(unsigned char where, unsigned int what,struct ringBufS *_ringBuff)
{
    FastTransfer_buffer_put(_ringBuff, where, what);
}
