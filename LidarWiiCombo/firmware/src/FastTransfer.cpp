/* 
 * File:   FastTransfer.cpp
 * Author: John
 * 
 * Created on January 27, 2017, 5:45 PM
 */

//#include <xc.h>
//#include <stdbool.h>
//#include <stdlib.h>
//#include "FastTransfer.hpp"

FastTransfer::FastTransfer() {
}

FastTransfer::FastTransfer(const FastTransfer& orig) {
}

FastTransfer::~FastTransfer() {
}

void FastTransfer::begin(int * ptr, unsigned char maxSize, unsigned char givenAddress, bool error, void (*stufftosend)(unsigned char), unsigned char (*stufftoreceive)(void),int (*stuffavailable)(void), unsigned char (*stuffpeek)(void))
{
...
bool FastTransfer::sendData(unsigned char whereToSend)
{
...
void FastTransfer::ToSend(unsigned char where, unsigned int what)
{
...
// disassembles the data and places it in a buffer to be sent
 ...
 void FastTransfer::FastTransfer_buffer_put(struct ringBufS *_this, unsigned char towhere, unsigned int towhat)
{
...
unsigned char FastTransfer::FastTransfer_buffer_get(struct ringBufS* _this)
{
    unsigned char c;
    if (_this->count > 0)
    {
...
void FastTransfer::FastTransfer_buffer_flush(struct ringBufS* _this, const int clearBuffer)
{
    _this->count = 0;
    _this->head = 0;
    _this->tail = 0;
...
unsigned int FastTransfer::FastTransfer_buffer_modulo_inc(const unsigned int value, const unsigned int modulus)
{
...
unsigned char FastTransfer::AKNAK(unsigned char module)
{
    int r;
...
void FastTransfer::crcBufS_put(struct crcBufS* _this, unsigned char address, unsigned char oldCRC, unsigned char status)
{
...
void FastTransfer::crcBufS_status_put(struct crcBufS* _this, unsigned char time, unsigned char status)
{
    if (time >= CRC_COUNT)
    {
        time = CRC_COUNT - 1;
...
unsigned char FastTransfer::crcBufS_get(struct crcBufS* _this, unsigned char time, unsigned char space)
{
    if (time >= CRC_COUNT)
    {
        time = CRC_COUNT - 1;
